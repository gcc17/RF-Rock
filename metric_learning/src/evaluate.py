...
def calculate_all_metrics(model: nn.Module,
    ...
    # if metrics["normalized_mutual_information"] > 1:
    #     print(metrics["normalized_mutual_information"])
    #     import os
    #     embedding_dir = f'embeddings_length8_f12_16_lo_1_{round(metrics["normalized_mutual_information"],2)}'
    #     os.makedirs(embedding_dir, exist_ok=True)
    #     train_set_size = embeddings_ref.size(1)
    #     test_set_size = embeddings_test.size(1)
    #     for ti in range(train_set_size):
    #         cur_label = labels_ref[ti]
    #         cur_label = cur_label.item()
    #         cur_embedding = embeddings_ref[0,ti,:].squeeze().cpu().numpy()
    #         np.save(os.path.join(embedding_dir, f'train_sample{ti}_label{cur_label}.npy'), cur_embedding)

    #     # for ti in range(test_set_size):
    #     #     cur_label = labels_test[ti]
    #     #     cur_label = cur_label.item()
    #     #     cur_embedding = embeddings_test[0,ti,:].squeeze().cpu().numpy()
    #     #     np.save(os.path.join(embedding_dir, f'test_sample{ti}_label{cur_label}.npy'), cur_embedding)
